...
const FEATURES = [
  { icon: '🧠', title: 'Semantic Analysis',    desc: 'SBERT understands meaning and context — not just matching words in answers.' },
  { icon: '⚡', title: 'Instant Grading',      desc: 'Grade a full class of submissions in seconds, with no queue.' },
  { icon: '📋', title: 'Rubric-based Scoring', desc: 'Evaluates accuracy, completeness, and clarity for consistent, fair results.' },
  { icon: '📈', title: 'Class Analytics',      desc: 'Dashboards for score distribution, progress trends, and at-risk students.' },
];

const STEPS = [
  { n: '01', title: 'Create an assessment',  desc: 'Add questions, set marks, and write sample answers for the AI grader.' },
  { n: '02', title: 'Share the access code', desc: 'Students join with a unique code — no email invites needed.' },
  { n: '03', title: 'AI grades instantly',   desc: 'SBERT scores each answer semantically. You review, override if needed.' },
  { n: '04', title: 'Analyse results',       desc: 'See score distributions, individual breakdowns, and class trends.' },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {/* ── Navbar ─────────────────────────────────────────────── */}
      <nav className={`sticky top-0 z-50 bg-white transition-shadow duration-300 ${scrolled ? 'shadow-sm border-b border-gray-100' : ''}`}>
      ...
      <section className="pt-20 pb-16 px-6 text-center overflow-hidden">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="hero-2 text-[52px] font-bold text-gray-950 leading-[1.12] mb-6 tracking-tight">
            Grading that understands<br />
            <TypingWord />
          </h1>

          <p className="hero-3 text-base text-gray-500 leading-relaxed mb-10 max-w-xl mx-auto">
            EvalAI uses Sentence-BERT to evaluate theory answers semantically — not just keyword matching. Instant, consistent, and fair at any scale.
          </p>
      ...
      <section id="features" className="py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <FadeSection className="text-center mb-14">
            ...
            <h2 className="text-3xl font-bold text-gray-950 mb-3">Everything you need to grade fairly</h2>
            <p className="text-sm text-gray-500 max-w-md mx-auto">Built for accuracy, transparency, and academic integrity — at any scale.</p>
          </FadeSection>
      ...
      <section id="how" className="py-24 px-6 bg-gray-950">
        <div className="max-w-4xl mx-auto">
          <FadeSection className="text-center mb-14">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">How it works</p>
            <h2 className="text-3xl font-bold text-white mb-3">Up and grading in four steps</h2>
            <p className="text-sm text-gray-400 max-w-md mx-auto">No complicated setup. Create an assessment and you're ready to go.</p>
          </FadeSection>

          <div className="grid sm:grid-cols-2 gap-5">
            {STEPS.map((s, i) => (
              <FadeSection key={s.n} delay={i * 0.1}
                className="border border-white/10 rounded-2xl p-6 hover:border-white/20 hover:bg-white/[0.03] transition-all duration-300">
                <div className="text-[11px] font-bold text-gray-600 uppercase tracking-widest mb-3">{s.n}</div>
      ...
      <section className="py-24 px-6" id="about">
        <div className="max-w-4xl mx-auto">
          <FadeSection className="text-center mb-12">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Who it's for</p>
            <h2 className="text-3xl font-bold text-gray-950">Built for both sides of the classroom</h2>
          </FadeSection>

          <div className="grid md:grid-cols-2 gap-6">
            {[
              {
                src: "lecturer-icon.svg",
              ...
              },
              {
                ...
                cta: 'Join an assessment', path: '/signup',
              },
            ].map((c, i) => (
              <FadeSection key={c.role} delay={i * 0.12}
                ...
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">{c.role}</p>
                <h3 className="text-xl font-bold text-gray-900 mb-3">{c.headline}</h3>
                <p className="text-sm text-gray-500 leading-relaxed flex-1 mb-6">{c.body}</p>
      ...
      <FadeSection className="py-20 px-6 bg-gray-950 text-center">
        <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Ready?</p>
        <h2 className="text-3xl font-bold text-white mb-4">Start grading with intelligence</h2>
        <p className="text-sm text-gray-400 mb-8 max-w-sm mx-auto">Join lecturers and students already using EvalAI. Free to get started.</p>
        <button onClick={() => navigate('/signup')}
      ...
      <footer className="bg-white border-t border-gray-100 py-8 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <p className="text-xs text-gray-400">&copy; {new Date().getFullYear()} EvalAI. All rights reserved.</p>
          <div className="flex gap-5">
            {['Terms', 'Privacy', 'Support'].map(l => (
              <a key={l} href={`#${l.toLowerCase()}`} className="text-xs text-gray-400 hover:text-gray-700 transition">{l}</a>
            ))}
          </div>
        </div>
      </footer>
