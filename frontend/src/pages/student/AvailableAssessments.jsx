...
export default function AvailableAssessments() {
  ...
  const handleJoin = async (e) => {
    ...
    if (assessment.status !== "Active") {
      setCodeError("This assessment is not currently active.");
      setJoining(false);
      return;
    }
      ...
      <div className="bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between sticky top-0 z-10">
        <div>
          ...
          <div className="text-xs text-gray-400">
            {loading
              ? "Loading…"
              : `${available.length} available · ${COMPLETED_MOCK.length} completed`}
          </div>
        </div>
      </div>
          ...
          <>
            ...
            {loading ? (
              ...
              available.map((a) => (
                <div
                  key={a.id}
                  className="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center justify-between hover:border-gray-300 hover:shadow-sm transition"
                >
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      {a.topic || "No topic"} · {a.questions} question
                      {a.questions !== 1 ? "s" : ""}
                    </div>
                  </div>
                  <button
        ...
        {activeTab === "Completed" &&
          (COMPLETED_MOCK.length === 0 ? (
            ...
            COMPLETED_MOCK.map((a) => (
              <div
                key={a.id}
                className="bg-white border border-gray-200 rounded-xl px-5 py-4 text-sm text-gray-700"
              >
                {a.title}
              </div>
            ))
